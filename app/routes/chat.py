...
async def chat(req: ChatRequest, request: Request):
    ...
    clinic = get_clinic(req.clinic_id)
    if not clinic:
    ...
    # Guardrails: emergencies and diagnosis/treatment
    ...
    if is_symptom_or_diagnosis_request(user_text):
        # no medical advice: guide to booking
        reply = (
            f"I can't provide medical advice or diagnose symptoms. "
            f"The safest step is to book an appointment so a clinician can asses you.\n\n"
    ...
    try:
        llm_reply = await chat_completion(system=system, messages=messages)
